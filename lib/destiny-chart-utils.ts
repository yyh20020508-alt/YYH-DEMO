 ...
 * Extracted from 命运图谱.html for React conversion.
 */
...
export function generate(form: FormInput): ReadingData {
    ...
    // yearBriefs — simplified version (age + year + score-based brief)
    const yearBriefs = line.map((it, i, l) => {
        ...
        const s = it.score,
            a = it.age,
            y = it.year;
        const pk = (a + y + Math.round(s)) % 7;
        const rising = prev ? s - prev.score >= 3 : false;
        const falling = prev ? s - prev.score <= -3 : false;
        const peaking = prev && next ? s - prev.score >= 2 && next.score - s <= -2 : false;
        const bottoming = prev && next ? s - prev.score <= -2 && next.score - s >= 2 : false;

        let text = '';
        if (a <= 18) {
            if (peaking) {
                text = `${a}岁运势到了一个小高峰，趁热多尝试！`;
            } else if (bottoming) {
                text = `${a}岁运势触底回升中，接下来会越来越好。`;
            } else if (s >= 85) {
                text = [`${a}岁状态全面在线，趁势多拓展。`, `${a}岁运势亮眼，把握机会。`][pk % 2];
            } else if (s >= 74) {
                text = [`${a}岁稳步前进，按自己节奏来。`, `${a}岁不温不火，但积累都有价值。`][pk % 2];
            } else {
                text = [
                    `${a}岁有点压力，调整心态最重要。`,
                    `${a}岁运势偏低，${rising ? '已经开始回升。' : '保持耐心。'}`
                ][pk % 2];
            }
        } else if (a <= 45) {
            if (peaking) {
                text = `${a}岁运势高峰，适合拍板做重要决定。`;
            } else if (bottoming) {
                text = `${a}岁触底反弹中，好运正在路上。`;
            } else if (s >= 88) {
                text = [`${a}岁运势拉满，${rising ? '还在涨！' : ''}该出手时别含蓄。`, `${a}岁手握好牌，大胆行动。`][
                    pk % 2
                ];
            } else if (s >= 76) {
                text = [
                    `${a}岁节奏不错，${falling ? '微降但仍在高位。' : '保持输出。'}`,
                    `${a}岁稳中有劲，守好基本盘。`
                ][pk % 2];
            } else if (s >= 68) {
                text = [`${a}岁节奏放缓，${rising ? '好在在好转。' : ''}蹲下来是为了跳更高。`][0];
            } else {
                text = `${a}岁运势偏低，守住基本盘等下一个上行周期。`;
            }
        } else {
            if (peaking) {
                text = `${a}岁迎来运势高峰，从容享受。`;
            } else if (bottoming) {
                text = `${a}岁运势转暖，轻松的日子不远了。`;
            } else if (s >= 85) {
                text = [`${a}岁越活越有味道，享受当下。`, `${a}岁运势不错，适合出去走走。`][pk % 2];
            } else if (s >= 74) {
                text = [`${a}岁平稳安逸，慢慢过最舒服。`, `${a}岁不急不躁，把时间给喜欢的事。`][pk % 2];
            } else {
                text = `${a}岁运势偏柔，${rising ? '已在回升。' : ''}注意保养，少操心多享福。`;
            }
        }
...
export function buildYearInsight(yearPt: LinePt, dm: DayMaster, scores: DimScores, st: Record<string, string>): string {
    ...
    if (a <= 3) {
        ...
        if (s >= 85) {
            ...
            advice = `<b>核心策略：快乐学习。</b>成绩重要但不是全部，身心健康排第一。${lo.label}偏弱（${sc[lo.key]}分），${lo.key === 'health' ? '注意别用眼过度，多运动' : lo.key === 'love' ? '多带孩子参加集体活动，培养社交' : '不急，慢慢来'}。`;
        } else {
