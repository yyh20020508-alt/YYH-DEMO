...
function DestinyChart() {
    ...
    const drawShareCanvas = useCallback(() => {
        ...
        const uname = (reading.name || '未命名') + '的命运图谱';
        ...
        let vibe = '',
            advice = '';
        if (sc >= 85) {
            vibe = `${yr}年运势在线，${a <= 18 ? '学什么都快' : a <= 45 ? '经验和运气共振' : '多年积累在兑现'}。`;
            advice = `${hi.label}维度最亮（${DSR[hi.key] as number}分），重点发力不亏。`;
        } else if (sc >= 70) {
            vibe = `${yr}年运势平稳，${a <= 18 ? '按部就班成长' : '稳中求进'}。`;
            advice = `${lo.label}稍弱（${DSR[lo.key] as number}分），注意补短板。`;
        } else {
            vibe = `${yr}年运势偏低调，调整蓄力为主。`;
            advice = `守住${hi.label}（${DSR[hi.key] as number}分）这个优势，等待时机。`;
        }
        const insTitle = `✦ ${yr}年度洞察`;
        const insParas = [vibe, advice];
        ...
        const ICARD = 16 + 16 + 8 + Math.max(iLines.length, 1) * ILH + 16;
        const BH = 28,
            CCARD = 16 + 14 + 6 + BH + 12 + 12 + 12;
        const CS = Math.floor((CW - 28 - 5 * 4) / 6),
            CGAP = Math.max(4, Math.floor((CW - 28 - 6 * CS) / 5));
        const LRH = CS * 2 + CGAP,
            LCARD = 16 + 14 + 6 + LRH + 12;
        const WCARD = 160;
        const LH = HH + G + RCARD + G + ICARD + G + CCARD + G + LCARD + G + WCARD + G + 32;
        cv.height = LH * DPR;
        ...
        // Background
        ctx.fillStyle = '#eef0f9';
        ctx.fillRect(0, 0, LW, LH);

        // Header — white bg, black text (like reference image)
        ctx.fillStyle = '#fff';
        ...
        ctx.fillText('DESTINY ATLAS', LW / 2, 22);
        ctx.letterSpacing = '0px';
        ctx.fillStyle = '#1c1c1e';
        ctx.font = F(26, 800);
        ctx.fillText(uname, LW / 2, 52);
        ctx.fillStyle = '#8e8e93';
        ctx.font = F(12, 400);
        ctx.fillText(`${pt.year}年运势 · ${grade}`, LW / 2, 70);
        // one-line motto from getNarr
        const mottoText = reading.narr?.p ? wrapLines(reading.narr.p.slice(0, 40), LW - 32, 1)[0] : '';
        if (mottoText) {
            ctx.fillStyle = '#3c3c43';
            ctx.font = F(12, 400);
            ctx.fillText(mottoText, LW / 2, 88);
        }
        ctx.textAlign = 'left';
        Y = HH + G;

        // Ring chart card
        drawCard(M, Y, CW, RCARD);
        ctx.fillStyle = '#1c1c1e';
        ctx.font = F(16, 700);
        ctx.fillText(`${pt.year}年运势详情`, M + 16, Y + 16 + 14);
        ctx.fillStyle = '#8e8e93';
        ctx.font = F(12, 400);
        ctx.fillText(`总分 ${pt.score} · ${grade}`, M + 16, Y + 16 + 14 + 4 + 12);
        ctx.textAlign = 'left';
        const RCTOP = Y + 16 + 18 + 4 + 12 + 12;
        const RCX = M + 16 + RD / 2,
            RCY = RCTOP + RCONTH / 2;
        const rScale = RD / 150;
        ...
        DIMS.forEach((d, i) => {
            ...
            // quip
            const qAvail = DW - 16 - 10;
            const ql = wrapLines(dimQuip(d.key, sv, a), qAvail, 1)[0] || '';
            ctx.fillStyle = '#8e8e93';
            ctx.font = F(11, 400);
            ctx.fillText(ql, DX + 16, ry + 27);
            // score — right-aligned in color
            ctx.textAlign = 'right';
            ctx.fillStyle = d.color[0];
            ctx.font = F(20, 800);
            ctx.fillText(String(sv), DX + DW, ry + 30);
            ctx.textAlign = 'left';
        });
        Y += RCARD + G;

        // Year insight card
        drawCard(M, Y, CW, ICARD);
        const ig = ctx.createLinearGradient(M, Y, M + CW, Y + ICARD);
        ig.addColorStop(0, 'rgba(99,102,241,0.05)');
        ig.addColorStop(1, 'rgba(236,72,153,0.03)');
        rrRect(M, Y, CW, ICARD, 14);
        ctx.fillStyle = ig;
        ctx.fill();
        ...
        ctx.fillStyle = '#4f46e5';
        ctx.font = F(13, 700);
        ctx.fillText(insTitle, M + 16, Y + 16 + 12);
        ctx.strokeStyle = 'rgba(99,102,241,0.10)';
        ctx.lineWidth = 1;
        ctx.beginPath();
        ctx.moveTo(M + 16, Y + 16 + 14);
        ctx.lineTo(M + CW - 16, Y + 16 + 14);
        ctx.stroke();
        ctx.fillStyle = '#374151';
        ctx.font = F(13, 400);
        iLines.forEach((l, i) => ctx.fillText(l, M + 16, Y + 16 + 16 + 8 + 14 + i * ILH));
        Y += ICARD + G;

        // Career phases card
        drawCard(M, Y, CW, CCARD);
        ctx.fillStyle = '#0f172a';
        ctx.font = F(13, 700);
        ctx.fillText(`${yr}年事业月度节奏`, M + 16, Y + 16 + 12);
        ...
        // Love heatmap card
        drawCard(M, Y, CW, LCARD);
        ctx.fillStyle = '#0f172a';
        ctx.font = F(13, 700);
        ctx.fillText(`${yr}年感情月度热力`, M + 16, Y + 16 + 12);
        ...
        mL.forEach((ml, i) => {
            const row = Math.floor(i / 6),
                col = i % 6;
            const lcx = LCELL_X + col * (CS + CGAP),
                lcy = LCELL_Y + row * (CS + CGAP);
            const t = (ml.score - lLo) / lRng;
            const alpha = (0.1 + t * 0.55).toFixed(2);
            rrRect(lcx, lcy, CS, CS, 8);
            ...
            ctx.font = F(9, 500);
            ctx.textAlign = 'center';
            ctx.fillText(`${ml.month}月`, lcx + CS / 2, lcy + CS * 0.38);
            ctx.font = F(15, 700);
            ctx.fillText(String(ml.score), lcx + CS / 2, lcy + CS * 0.76);
            ctx.textAlign = 'left';
        });
        Y += LCARD + G;

        // Wealth K-line card
        drawCard(M, Y, CW, WCARD);
        ctx.fillStyle = '#0f172a';
        ctx.font = F(13, 700);
        ctx.fillText(`${yr}年财富月度K线`, M + 16, Y + 16 + 12);
        ...
        ctx.font = F(10, 400);
        ctx.textAlign = 'center';
        ctx.fillText('DESTINY ATLAS · 命运图谱', LW / 2, Y + 16);
        ctx.textAlign = 'left';
        cv.style.width = '100%';
