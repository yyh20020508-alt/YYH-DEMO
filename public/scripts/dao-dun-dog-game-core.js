...
function getSwordSegment(entity) {
  ...
  return {
    x1: entity.x + ux * (orbit - blade * 0.2),
    y1: entity.y + uy * (orbit - blade * 0.2),
    x2: entity.x + ux * (orbit + blade * 0.95),
    y2: entity.y + uy * (orbit + blade * 0.95),
    blade
  };
}
...
function update() {
  ...
  for (const e of G.enemies) {
    ...
    const swordHit = pointSegDist(p.x, p.y, swordSeg.x1, swordSeg.y1, swordSeg.x2, swordSeg.y2) <= p.r + swordSeg.blade * 0.18;
    if (p.shieldActive) {
...
function drawHUD(W, H) {
  ...
  ctx.fillText(`❤️ ${Math.max(0,p.hp)} / ${p.maxHp}`, 16 + 170 / 2, st + 22 / 2 + 1);
  ctx.shadowBlur = 0;
  ...
  ctx.fillText(`⚔️ Lv.${p.weaponLevel}  ${p.xp}/${p.xpNext} XP`, 16 + 170 / 2, st + 26 + 14 / 2 + 1);
  ctx.shadowBlur = 0;
  ctx.textBaseline = 'alphabetic';
