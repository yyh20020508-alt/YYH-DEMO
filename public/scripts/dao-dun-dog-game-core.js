...
const playerImgs = Array.from({length: 7}, (_, i) => { const img = new Image(); img.src = `./player${i+1}.png`; return img; });
// 7 enemy tiers: level 1-2 → img1, 3-4 → img2, 5-6 → img3, 7-9 → img4, 10-12 → img5, 13-15 → img6, 16+ → img7
const enemyImgs = Array.from({length: 7}, (_, i) => { const img = new Image(); img.src = `./enemy${i+1}.png`; return img; });
// Weapon images: 刀1-7 (sword), 盾1-7 (shield)
const swordImgs  = Array.from({length: 7}, (_, i) => { const img = new Image(); img.src = `./sword${i+1}.png`; return img; });
const shieldImgs = Array.from({length: 7}, (_, i) => { const img = new Image(); img.src = `./shield${i+1}.png`; return img; });
// Obstacle images: 障碍1-7
const obstacleImgs = Array.from({length: 7}, (_, i) => { const img = new Image(); img.src = `./obstacle${i+1}.png`; return img; });
