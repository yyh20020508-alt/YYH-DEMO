...
function swordBtnPos()  { return { x: window.innerWidth - 84, y: window.innerHeight - 150 - safeBottom, r: 42 }; }
function shieldBtnPos() { return { x: window.innerWidth - 84, y: window.innerHeight - 258 - safeBottom, r: 42 }; }
...
function getSwordSegment(entity) {
  const orbit = weaponOrbit(entity.weaponLevel);
  const blade = weaponBlade(entity.weaponLevel);
  const ux = Math.cos(entity.weaponAngle);
  const uy = Math.sin(entity.weaponAngle);
  return {
    x1: entity.x + ux * (orbit - blade * 0.45),
    y1: entity.y + uy * (orbit - blade * 0.45),
    x2: entity.x + ux * (orbit + blade * 1.05),
    y2: entity.y + uy * (orbit + blade * 1.05),
    blade
  };
}
...
function drawCenteredBoxText(text, x, y, w, h, font) {
  ctx.save();
  ctx.font = font;
  ctx.textAlign = 'left';
  ctx.textBaseline = 'alphabetic';
  ...
  const tx = x + w / 2 - (left + right) / 2;
  const ty = y + h / 2 + (ascent - descent) / 2;
...
function update() {
  ...
  for (let i = G.enemies.length - 1; i >= 0; i--) {
    ...
    const swordSeg = getSwordSegment(p);
    const swordHit = pointSegDist(e.x, e.y, swordSeg.x1, swordSeg.y1, swordSeg.x2, swordSeg.y2) <= e.r + swordSeg.blade * 0.32;
    if (swordHit) {
      e.hp--;
      e.invincible = 25;
      sfxHit();
  ...
  for (const e of G.enemies) {
    ...
    const swordSeg = getSwordSegment(e);
    const swordHit = pointSegDist(p.x, p.y, swordSeg.x1, swordSeg.y1, swordSeg.x2, swordSeg.y2) <= p.r + swordSeg.blade * 0.32;
    if (p.shieldActive) {
  ...
  for (const e of G.enemies) {
    ...
    if (e.state === 'chase') {
      ...
      const spreadR = d > 520 ? 24 : 60; // farther away -> run more directly at player
    ...
    if (len > 1) {
      const chaseBoost = d > 900 ? 1.28 : d > 650 ? 1.14 : 1;
      e.vx = (tx / len) * spd * chaseBoost; e.vy = (ty / len) * spd * chaseBoost;
...
function drawHUD(W, H) {
  ...
  drawCenteredBoxText(`${Math.max(0,p.hp)} / ${p.maxHp} HP`, 16, st, 170, 22, 'bold 11px sans-serif');
  ctx.shadowBlur = 0;
  ...
  drawCenteredBoxText(`Lv.${p.weaponLevel}  ${p.xp}/${p.xpNext} XP`, 16, st + 26, 170, 14, '10px sans-serif');
  ctx.shadowBlur = 0;
