...
function update() {
  ...
  for (let i = G.enemies.length - 1; i >= 0; i--) {
    ...
    const swordHit = pointSegDist(e.x, e.y, swordSeg.x1, swordSeg.y1, swordSeg.x2, swordSeg.y2) <= e.r + Math.max(12, swordSeg.blade * 0.5);
    if (swordHit) {
      e.hp--;
      e.invincible = 25;
      sfxHit();
  ...
  for (const e of G.enemies) {
    ...
    const swordHit = pointSegDist(p.x, p.y, swordSeg.x1, swordSeg.y1, swordSeg.x2, swordSeg.y2) <= p.r + Math.max(12, swordSeg.blade * 0.5);
    if (p.shieldActive) {
...
function drawHUD(W, H) {
  ...
  ctx.fillText(`${Math.max(0,p.hp)} / ${p.maxHp} HP`, 16 + 170 / 2, st + 22 / 2);
  ctx.shadowBlur = 0;
  ...
  ctx.fillText(`Lv.${p.weaponLevel}  ${p.xp}/${p.xpNext} XP`, 16 + 170 / 2, st + 26 + 14 / 2);
  ctx.shadowBlur = 0;
  ctx.textBaseline = 'alphabetic';
